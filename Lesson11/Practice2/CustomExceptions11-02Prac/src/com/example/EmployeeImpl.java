...
public class EmployeeImpl {
    ...
    public void add(Employee emp) throws InvalidOperationException {
        ...
        if (employeeArray[emp.getId()] != null) {
            throw new InvalidOperationException("Error adding employee. Employee id: " + emp.getId() + " already exists.");
        }

        try {
            employeeArray[emp.getId()] = emp;
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new InvalidOperationException("Error adding employee. Emp id must be less than: " + employeeArray.length);
        }
    }
    ...
    public void delete(int id) throws InvalidOperationException {
        ...
        if (employeeArray[id] == null) {
            throw new InvalidOperationException("Error deleting employee. Employee id: " + id + " does not exist.");
        }

        try {
            employeeArray[id] = null;
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new InvalidOperationException("Error deleting employee. Emp id must be less than " + employeeArray.length);
        }
    }
